...
%   oooooooo   ooooo oo     oo  oo   ooo  oooooooo oooooooo     ooo
    ...
    closet.vector_coat_sizes_mu = round(getRandomNumberVec(25, 25, closet.N_particles));
    closet.displace_in_order = 0;                                          % Boolean variable to decide if the coating is random (0) or not (1)
    
    % Cone of inspection features
    closet.fixed_direction = 0;                                            % Boolean variable to set a fixed direction for the central axis of the conee
    closet.cone_aperture_degree = 20;                                     % Cone aperture of inspection: [deg]
    closet.N_raysXSA = 1;                                                  % Number of rays per each solid angle
    
    % Euler angles 
    closet.N_Euler_triplets = 1;                                           % Number of angles per fixed direction within the solid angle
    ...
    closet.in_const = 1.1;                                                 % Constant that multiplies the initial position of the coarse movement
    ...
    [closet]  = initializeEmptyMatrices(closet);
     ...
     for id_particle = 1:closet.N_particles
         ...
         for id_single_ray = 1:closet.N_raysXSA
             ...
             if (closet.origin_in_the_CM >0) && (c_Object.CM_alert<0)
             ...
             parfor id_angle = 1:closet.N_Euler_triplets                   % parallel loop
                ...
                vec_rotation = [getRandomNumber(0, 360), ...
                ...
                p_upper_x = start_p.x + closet.in_const * closet.vector_coat_sizes_mu(id_particle) * x_dir;
                p_upper_y = start_p.y + closet.in_const * closet.vector_coat_sizes_mu(id_particle) * y_dir;
                p_upper_z = start_p.z + closet.in_const * closet.vector_coat_sizes_mu(id_particle) * z_dir;
...
function central_Object = createCObject(fv, sphere_struct)
    ...
    IN = inpolyhedron(fv, [x_CM, y_CM, z_CM]);
    
    if (IN == 1)
        central_Object.CM_alert = 0;
    else
        central_Object.CM_alert = 1;
    end
...
function output = solidAngleRndPoints(coneAngleDegree, coneDir, N, RNG)
    ...
    if ((coneDir(1)==0) && (coneDir(2)==0) && (coneDir(3)==1))
        coneDir(3) = 0.99999;
        coneDir(1) = sqrt(1-coneDir(3).^2);
        coneDir(2) = 0;
    end
